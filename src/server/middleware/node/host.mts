...
const verifyBody = async (req, res, next) => {
  try {
    ...
    console.log('identifierInStore', identifierInStore);
    ...
    if (identifier !== identifierInStore) {
      return res.status(403).json({ error: 'Logged in as a different user' });
    }
    ...
    // subscribedPortraits is an array of portraitIds, check if the portraitId is already in the array
    if (subscribedPortraits.includes(portraitId)) {
      return res.status(400).json({ error: 'Portrait already hosted' });
    }
